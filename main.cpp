...
int main() {
    ...
    while (window.isOpen()) {
        ...
        for (auto& p : particles) {
            ...
            if (p.pos.x - p.radius <= 0) { p.pos.x = p.radius + 0.1f; p.vel.x = abs(p.vel.x); }
            if (p.pos.x + p.radius >= width) { p.pos.x = width - p.radius - 0.1f; p.vel.x = -abs(p.vel.x); }
            if (p.pos.y - p.radius <= 0) { p.pos.y = p.radius + 0.1f; p.vel.y = abs(p.vel.y); }
            if (p.pos.y + p.radius >= height) { p.pos.y = height - p.radius - 0.1f; p.vel.y = -abs(p.vel.y); }
            
            float minSpeed = 0.5f;
            if (abs(p.vel.x) < minSpeed && abs(p.vel.y) < minSpeed) {
